...
**4.21 — Backup d'app dans `custom_apps/` casse `occ upgrade`.** Au déploiement v0.3.5 en prod, j'avais nommé le backup `custom_apps/lists.backup-20260530-221532` avant d'extraire la nouvelle version. `occ upgrade` a fait les migrations DB avec succès puis crashé au "code integrity check" : `OCP\App\AppPathNotFoundException: Could not find path for listsbackup-20260530-221532`. NC scanne tous les sous-dossiers de `custom_apps/` comme apps candidates, et le parser d'app id strip les `.` (donc `lists.backup-…` devient l'app id `listsbackup-…`). Conséquence : maintenance mode laissé actif, intervention manuelle pour désactiver. **Toujours stocker les backups d'app HORS de `custom_apps/`** — j'utilise `data/lists-backups/` (le volume `nc_data` est persistent et hors du scan).

**4.20 — `NcActions` ne rend pas son trigger en @vue/compat MODE:2.** `NcActions.render()` filtre les slot children via `getActionName(action)` qui lit `action.componentOptions.Ctor.extendOptions.name` ?? `action.componentOptions.tag` — API VNode Vue 2 pure. Les VNodes émis par le compiler compat n'ont pas cette structure → tous les `<NcActionButton>` sont filtrés out → `actions.length === 0` → `render()` retourne `undefined` → aucun bouton ⋮ ne s'affiche. Workaround : `<button>` natif + `<div v-if="openMenuId === X">` manuel comme dropdown (cf. `App.vue` listing menu). Idem que [[4.19]] mais sur un autre composant.
